...
function safeFileName(name) {
  ...
  return /\.(mid|midi)$/i.test(clean) ? clean : clean + '.mid';
}
...
const server = http.createServer(async (req, res) => {
  ...
  // ── Save one-shot DAW export file and optionally reveal in Finder ──
  // POST /api/export-file?name=file.mid&reveal=1  body=bytes
  if (pathname === '/api/export-file' && req.method === 'POST') {
    const safeName = safeFileName(qs.name || 'export.mid');
    if (!safeName) return err(res, 400, 'bad name');
    const outPath = path.join(EXPORTS, safeName);
    try {
      const body = await readBody(req);
      fs.mkdirSync(EXPORTS, { recursive: true });
      fs.writeFileSync(outPath, body);
      if (String(qs.reveal || '') === '1') {
        try { cp.spawn('open', ['-R', outPath], { detached: true, stdio: 'ignore' }).unref(); } catch {}
      }
      json(res, { ok: true, savedAs: outPath, fileName: safeName });
    } catch(e) { err(res, 500, e.message); }
    return;
  }

  // ── Reveal DAW exports folder in Finder ──
  // GET /api/reveal-exports
  if (pathname === '/api/reveal-exports') {
    try {
      fs.mkdirSync(EXPORTS, { recursive: true });
      cp.spawn('open', [EXPORTS], { detached: true, stdio: 'ignore' }).unref();
      return json(res, { ok: true, folder: EXPORTS });
